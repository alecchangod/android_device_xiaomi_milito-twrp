...
#define HW_V "ro.boot.hwversion"

void search_variant(const std::vector<variant_info_t> variants) {
    std::string hwc_value = GetProperty(HWC_PROP, "");
    std::string sku_value = GetProperty(SKU_PROP, "");
    std::string hwversion = GetProperty(HW_V, "");

    for (const auto& variant : variants) {
        if ((variant.hwc_value == hwc_value || variant.hwc_value == "" ) &&
            (variant.sku_value == sku_value || variant.sku_value == "" ) &&
            (variant.hwversion == hwversion || variant.hwversion == "" )) {
...
void set_variant_props(const variant_info_t variant) {
    ...
    property_override("ro.matrixx.battery", variant.battery_size, true);
